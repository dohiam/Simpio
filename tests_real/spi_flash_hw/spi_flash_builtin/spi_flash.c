...
#define W25Q_READ_MANUFACTURER_ID       0X90
...
uint16_t __not_in_flash_func(read_manufacturer_id)(spi_inst_t *spi, uint cs_pin) {
    union { uint8_t ids[2]; uint16_t id; } idu;
    uint8_t dummy[] = {0,0};
    uint8_t cmdbuf[4] = {
            W25Q_READ_MANUFACTURER_ID,
            0,
            0,
            0
    };
    ...
    cs_select(cs_pin);
    spi_write_blocking(spi, cmdbuf, 4);
    spi_write_read_blocking(spi, dummy, idu.ids, 2);
    cs_deselect(cs_pin);
    //flash_wait_done(spi, cs_pin);
    return idu.id;
}
...
#warning spi/spi_flash example requires a board with SPI pins
    ...
    uint16_t id = read_manufacturer_id(spi_default, PICO_DEFAULT_SPI_CSN_PIN);
